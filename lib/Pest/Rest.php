...
class Pest_Rest {
  ...
  protected function checkLastResponseForError() {
    ...
    switch ($meta['http_code']) {
      case 400:
        throw new Pest_Exception_BadRequest($this->processError($body));
        break;
      case 401:
        throw new Pest_Exception_Unauthorized($this->processError($body));
        break;
      case 403:
        throw new Pest_Exception_Forbidden($this->processError($body));
        break;
      case 404:
        throw new Pest_Exception_NotFound($this->processError($body));
        break;
      case 405:
        throw new Pest_Exception_MethodNotAllowed($this->processError($body));
        break;
      case 409:
        throw new Pest_Exception_Conflict($this->processError($body));
        break;
      case 410:
        throw new Pest_Exception_Gone($this->processError($body));
        break;
      case 422:
        ...
        throw new Pest_Exception_InvalidRecord($this->processError($body));
        break;
      default:
        if ($meta['http_code'] >= 400 && $meta['http_code'] <= 499)
          throw new Pest_Exception_ClientError($this->processError($body));
        elseif ($meta['http_code'] >= 500 && $meta['http_code'] <= 599)
          throw new Pest_Exception_ServerError($this->processError($body));
        elseif (!$meta['http_code'] || $meta['http_code'] >= 600) {
          throw new Pest_Exception_UnknownResponse($this->processError($body));
        }
    }
  }
}